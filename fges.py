...
class FGES:
    def __init__(self, variables, score, filename='', checkpoint_frequency=0, score_type='bic',
                 save_name=None, knowledge=None, verbose=False, max_parents=None):
        self.top_graphs = []
    ...
    def get_dict(self):
        return {"graph": self.graph,
                "adjacency": nx.linalg.graphmatrix.adjacency_matrix(self.graph).todense(),
    ...
    def search(self):
        ...
        if self.graph is None:
            ...
            # my: add edges from knowledge. not used.
        ...
        if not self.in_bes:
            ...
            self.arrow_dict = {}
            self.stored_neighbors = {}
            print('===> Finish forward stage with ' + str(len(self.graph.edges())) + ' edges.\n')
    ...
    def fes(self):
        ...
        while len(self.sorted_arrows) > 0:
            ...
            # Max number of parents check
            if self.max_parents is not None:
                if graph_util.num_parents(self.graph, y) >= self.max_parents:
                    continue
                ...
                if bump > 0:
                    if self.verbose:
                        print("Evaluated removal of an arrow " + str(
                            a) + " -> " + str(b) + " with diff " + str(diff) + " and bump: " + str(bump))
    ...
    def insert_eval(self, x, y, T, na_y_x):
        ...
        _na_y_x = set(na_y_x)
        _na_y_x.update(T)
        _na_y_x_ = _na_y_x.copy()
        _na_y_x.update(graph_util.get_parents(self.graph, y)) # Pa_G (excluding X)
